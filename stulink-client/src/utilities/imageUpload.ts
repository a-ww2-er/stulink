...
export const imageUpload = async (file: string | Blob) => {
    const data = new FormData();
    data.append("file", file);
    data.append("upload_preset", "stulink-profile");
    try {
      const res = await axios.post("https://api.cloudinary.com/v1_1/dsxbt6phu/image/upload", data);
      const { url } = res.data;
      return url;
    } catch (error:any) {
...
// import { FormikValues, useFormik, withFormik } from "formik";
...
//     isSubmitting,
//   } = props;
//   const { error, setErrors, formIndex, setFormIndex } = useContext(AppContext);
...
//               <AnimatedForm>
//                 {RegisterFormData?.[formIndex]?.[1].map(
//                   (items: form_object) => {
//                     if (items.group) {
//                       return (
...
//                                     >
//                                       <input
//                                         className={groupitems.className}
...
//                               {items.label}{" "}
//                             </article>
//                             {touched[items.name as keyof typeof touched] &&
//                             errors[items.name as keyof typeof errors] ? (
//                               <article className="form_validation_errors">
//                                 {errors[items.name as keyof typeof errors]}
//                               </article>
//                             ) : (
...
//                             }
//                             isSearchable={true}
//                             value={values[items.name as keyof typeof values]}
//                             name={items.name}
//                             placeholder={<DropdownPlaceholder item={items} />}
//                           />
//                         </aside>
//                       );
//                     }
...
//   //   const { email, password } = values;
//   //   React.useContext(AppContext).setIsLoading(true);
//   //   console.log(values);
...
//   //       password,
//   //     });
//   //     React.useContext(AppContext).setIsLoading(false);
//   //     React.useContext(AppContext).setErrors("");
//   //     localStorage.setItem("currentUser", JSON.stringify(res.data));
...
//     <>
//       <motion.div
//         key={JSON.stringify(children, ignoreCircularReferances())}
//         initial={{ opacity: 0.5, x: 200 }}
//         animate={{ opacity: 1, x: 0 }}
//         exit={{ opacity: 0, x: -200 }}
//         transition={bounceTransition}
